...
std::istream & operator>>( std::istream & stream, GroceryItem & groceryItem )
{
  ...
  // char comma;
  // stream >>
  // std::ws >> std::quoted(groceryItem._upcCode)      >> comma >>
  // std::ws >> std::quoted(groceryItem._brandName)    >> comma >>
  // std::ws >> std::quoted(groceryItem._productName)  >> comma >>
  // std::ws >> groceryItem._price;
  // return stream;
  std::string upcCode, brand, product;
  char comma;
  double price;
  if (stream >> std::quoted (upcCode) >> comma >> std::quoted( brand ) >> comma >> std::quoted( product ) >> comma >> price )
  {
    groceryItem.upcCode(std::move(upcCode));
...
std::ostream & operator<<( std::ostream & stream, const GroceryItem & groceryItem )
{
  ...
  // return stream << std::quoted(groceryItem._upcCode) << comma << std::quoted(groceryItem._brandName) << comma << std::quoted(groceryItem._productName) << comma << groceryItem._price;
  /////////////////////// END-TO-DO (22) ////////////////////////////
}
